...
class AlphaVantageETL(SparkETL):
    ...
    def combine_data(self, symbol_list: List[str], api_key: str):
        ...
        for symbol in symbol_list:
            ...
            if data:
                combined_data = data if combined_data is None else combined_data.union(data)
    ...
    def transform(self, df_combined):
        ...
        """
        df_combined = df_combined.withColumn("avg", ((df_combined.open*df_combined.close)/2).cast('decimal(18,4)'))
        df_combined = df_combined.withColumn("pk", (concat(col("symbol"), col("week_from"))))
